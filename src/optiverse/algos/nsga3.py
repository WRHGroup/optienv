...
def _fast_non_dominated_sort(F: np.ndarray) -> List[np.ndarray]:
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            Fj = F[j]
            # dominance compare (weak <= in all & strict < in at least one)
...
def _sbx_crossover(
    p1: np.ndarray, p2: np.ndarray, lb: np.ndarray, ub: np.ndarray,
    eta_c: float = 30.0, p_c: float = 1.0, rng: Optional[np.random.Generator] = None
...
def _poly_mutation(
    x: np.ndarray, lb: np.ndarray, ub: np.ndarray,
    eta_m: float = 20.0, p_m: Optional[float] = None, rng: Optional[np.random.Generator] = None
) -> np.ndarray:
    rng = rng or np.random.default_rng()
    n = x.size
    if p_m is None:
    ...
    # Ensure at least one dimension mutates (prevents cloning when r>p_m for all j)
    if not mutated:
        j = int(rng.integers(0, n))
        u = rng.random()
        if u < 0.5:
            deltaq = (2.0 * u) ** (1.0 / (1.0 + eta_m)) - 1.0
        else:
            deltaq = 1.0 - (2.0 * (1.0 - u)) ** (1.0 / (1.0 + eta_m))
        span = ub[j] - lb[j]
        y[j] = np.clip(y[j] + deltaq * span, lb[j], ub[j])
...
class NSGA3:
    ...
    def __init__(
        ...
        self.p_c = float(p_c)
        self.p_m = p_m
        self.rng = np.random.default_rng(seed)  # persistent RNG, never reseed
    ...
    def initialize(self, n_var: int, bounds: List[Tuple[float, float]]) -> np.ndarray:
        ...
        self._bounds = (lb, ub)
        if self.p_m is None:  # set default given n_var
            self.p_m = 1.0 / n_var
        return pop

    def ask(self, pop: np.ndarray, fit: np.ndarray, bounds: List[Tuple[float, float]]) -> np.ndarray:
        # Variation: binary tournament by rank (here proportional to 1/(1+rank)).
        n, d = pop.shape

        lb = np.array([b[0] for b in bounds], dtype=float)
        ub = np.array([b[1] for b in bounds], dtype=float)

        # Use survivor-aligned ranks if present
        if hasattr(self, "_rank_pop"):
            inv = 1.0 / (1.0 + self._rank_pop.astype(float))
            prob = inv / inv.sum()
            parents_idx = self.rng.choice(n, size=(n,), p=prob)
        else:
            # first ask after initialize/resume: pure random mating
            parents_idx = self.rng.integers(0, n, size=(n,))

        children = []
        for i in range(0, n, 2):
            i1 = int(parents_idx[i % n])
            i2 = int(parents_idx[(i + 1) % n])
            if i2 == i1:
                # Avoid trivial SBX clones: bounded retries to draw a different mate
                for _ in range(3):
                    jtry = int(self.rng.integers(0, n))
                    if jtry != i1:
                        i2 = jtry
                        break
    ...
    def tell(
        self, pop: np.ndarray, fit: np.ndarray, new_pop: np.ndarray, new_fit: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self._rank = self._rank_from_fronts(fronts, F.shape[0])   # ranks for 2N
        self._rank_pop = self._rank[next_idx]                     # survivor-aligned ranks
    ...
    def _niching_select(
        self, F_all: np.ndarray, fronts: List[np.ndarray],
        selected_idx: List[int], split_front: np.ndarray, k: int
    ) -> np.ndarray:
        """
        ...
        # add tiny jitter to break ties stochastically
        split_dist = split_dist + 1e-12 * self.rng.random(split_dist.shape[0])
        ...
        order = self.rng.permutation(len(split_front))  # randomize insertion order (avoid parents-first bias)
        for idx_local in order:
            idx_global = split_front[idx_local]
            j = split_dirs[idx_local]
            d = split_dist[idx_local]
        ...
        for _ in range(k):
            ...
            if not eligible:
                # fallback: no candidates left (e.g., due to earlier picks)
            ...
            j_pick = int(self.rng.choice(J))

            lst = cand_per_niche[j_pick]
            if niche_count[j_pick] == 0:
                # choose smallest perp distance (with jitter this breaks ties)
                lst.sort(key=lambda t: t[1])
                idx_global, _ = lst.pop(0)
            else:
                # random selection within niche to maintain diversity
                r = int(self.rng.integers(0, len(lst)))
